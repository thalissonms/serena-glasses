...
import { subcategoryPatchSchema } from "@features/admin/schemas/category.schema";
...
export const PATCH = withAdmin<{ id: string }>(async (req, { params }) => {
  ...
  const { error } = await getSupabaseServer().from("subcategories").update(parsed.data).eq("id", id);
