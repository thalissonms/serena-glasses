...
export const Input = forwardRef<HTMLInputElement, Props>(function Input(
  ...
  return (
    <div className="flex flex-col gap-1.5 w-full">
      ...
      >
        {prefix && (
          <span className="pl-3 text-white/25 flex-shrink-0 flex items-center">{prefix}</span>
        )}
        <input
          ref={ref}
          id={inputId}
        ...
        />
        {suffix && (
          <span className="pr-3 text-white/25 flex-shrink-0 flex items-center">{suffix}</span>
        )}
      </div>
      {error && (
