 ...
 *
 * Usado em: /admin-v2/coupons/new e /admin-v2/coupons/[id].
 */
...
function Toggle({
  ...
  return (
    ...
    >
      <span
        className={`relative w-10 h-5 border transition-all duration-200 flex-shrink-0 ${
...
function RadioOption<T extends string>({
  ...
  return (
    ...
    >
      <span
        className={`flex-shrink-0 w-3 h-3 border rounded-full transition-all duration-150 ${
          selected
      ...
      />
      {icon && (
        <span className={`flex-shrink-0 ${selected ? "text-[#FF00B6]" : "text-white/25"}`}>
          {icon}
        </span>
      )}
        ...
        {filtered.length === 0 ? (
          ...
          filtered.map((p) => {
            ...
            return (
              ...
              >
                <span
                  className={`flex-shrink-0 w-3 h-3 border transition-all ${
                    isSel
                ...
                {p.code && (
                  <span className="font-mono text-[8px] text-white/20 flex-shrink-0">{p.code}</span>
                )}
              </button>
            );
          })
  ...
  async function onSubmit(data: FormData) {
    ...
    try {
      ...
      if (mode === "create" && result.id) {
        router.push(`/admin-v2/coupons/${result.id}`);
      } else {
        router.refresh();
      }
    } catch {
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-8">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <button
            type="button"
            onClick={() => router.push("/admin-v2/coupons")}
