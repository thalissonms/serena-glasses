 ...
 *
 * Usado em: src/app/admin-v2/orders/page.tsx.
 */
...
import { DateRangePicker } from "@features/admin-v2/components/primitives/DateRangePicker";
...
const STATUS_CHIPS = [
  ...
  { value: "paid", label: "PAGO", color: "#00F0FF" },
  { value: "processing", label: "PROCESSANDO", color: "#3B82F6" },
  { value: "shipped", label: "ENVIADO", color: "#FF00B6" },
...
const STATUS_ROW_STYLE: Record<string, { bg: string; border: string; hoverBg: string; hoverBorder: string }> = {
  ...
  paid:           { bg: "",               border: "border-l-[#00F0FF]/20",   hoverBg: "hover:bg-[#00F0FF]/5",    hoverBorder: "hover:border-l-[#00F0FF]/50"  },
  processing:     { bg: "",               border: "border-l-blue-500/20",    hoverBg: "hover:bg-blue-500/5",     hoverBorder: "hover:border-l-blue-500/50"   },
  shipped:        { bg: "",               border: "border-l-[#FF00B6]/20",   hoverBg: "hover:bg-[#FF00B6]/5",    hoverBorder: "hover:border-l-[#FF00B6]/50"  },
...
function PaymentCell({ method }: { method: string }) {
  if (method === "pix") {
    return (
      <span className="inline-flex items-center gap-1.5">
        ...
        <span className="font-mono text-[9px] text-white/40">PIX</span>
      </span>
    );
  }
  if (method === "credit_card") {
    return (
      <span className="inline-flex items-center gap-1.5">
        ...
        <span className="font-mono text-[9px] text-white/40">Cartão</span>
      </span>
    );
  }
  if (method === "boleto" || method === "boleto_bancario") {
    return (
      <span className="inline-flex items-center gap-1.5">
        <FileText size={10} className="text-[#FFD700]" />
        <span className="font-mono text-[9px] text-white/40">Boleto</span>
      </span>
    );
  }
  if (method === "mercadopago") {
    return (
      <span className="inline-flex items-center gap-1.5">
        <DollarSign size={10} className="text-[#00B1EA]" />
        <span className="font-mono text-[9px] text-white/40">MercadoPago</span>
      </span>
    );
  }
  return <span className="font-mono text-[9px] text-white/25">{method ?? "—"}</span>;
}
...
export function OrdersListClient({
  ...
  function buildAndPush(opts: {
    ...
    router.push(`/admin-v2/orders${qs ? "?" + qs : ""}`);
  }
    ...
    <div className="flex flex-col min-h-full">
      ...
      <div className="flex items-center justify-between px-6 py-4 border-b border-white/5 shrink-0">
        <div className="flex items-center gap-3">
          <ShoppingBag size={16} className="text-[#FF00B6]" />
          <h1 className="font-[Shrikhand] text-2xl text-white tracking-wider">PEDIDOS</h1>
          <span className="font-mono text-[9px] uppercase tracking-widest text-white/20 border border-white/8 px-2 py-0.5">
            {result.total.toLocaleString("pt-BR")} total
          </span>
          {initialStatus && (
            <span className="font-mono text-[8px] uppercase tracking-widest text-white/15 border border-white/5 px-2 py-0.5">
              filtrado
            </span>
          )}
        </div>
        <span className="font-mono text-[8px] uppercase tracking-widest text-white/15 border border-white/5 px-3 py-1.5">
          Pedidos entram pelo storefront
        </span>
      </div>

      {/* ── Sticky filter bar ── */}
      <div className="sticky top-0 z-20 bg-[#0f0f0f]/95 backdrop-blur-sm border-b border-white/5 px-6 pt-3 pb-3 shrink-0">
        ...
        <div className="flex items-end gap-3 flex-wrap mb-3">
          ...
          {hasFilters && (
            <button
              ...
              className="self-end flex items-center gap-1.5 font-mono text-[9px] uppercase tracking-widest text-white/25 hover:text-[#FF00B6] border border-white/5 hover:border-[#FF00B6]/30 px-3 py-2.5 transition-all duration-150"
            >
              <X size={9} />
              Limpar
            </button>
          ...
          {STATUS_CHIPS.map((chip) => {
            ...
            return (
              <button
                ...
                style={activeStyle}
                className={clsx(
                  "font-mono text-[8px] uppercase tracking-[0.2em] px-2.5 py-1 border transition-all duration-150",
                  isActive && !chip.color
                    ? "border-[#FF00B6] text-[#FF00B6] bg-[#FF00B6]/8 shadow-[0_0_8px_rgba(255,0,182,0.25)]"
                    : !isActive
      ...
      {/* ── Bulk action bar ── */}
      {selectedRows.size > 0 && (
        <div className="shrink-0 bg-[#1a1a1a] border-b border-[#FF00B6]/15 px-6 py-2 flex items-center gap-4">
          <span className="font-mono text-[9px] uppercase tracking-widest text-[#FF00B6]">
        ...
        {visibleOrders.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 gap-4">
            ...
            {hasFilters && (
              <button
                onClick={handleClearAll}
                className="font-mono text-[9px] uppercase tracking-widest text-[#FF00B6] hover:underline transition-all"
              >
                Limpar filtros
              </button>
            )}
                ...
                <tr className="bg-[#1a1a1a] shadow-[inset_1px_1px_0_rgba(255,255,255,0.05),inset_-1px_-1px_0_rgba(0,0,0,0.3)]">
                  <th className="px-4 py-3 w-8 border-b border-white/5">
                    <button onClick={toggleAll} className="text-white/20 hover:text-white/50 transition-colors">
                      {allSelected
                        ? <CheckSquare size={13} className="text-[#FF00B6]" />
                        : <Square size={13} />
                      }
                    </button>
                  </th>
                  ...
                  return (
                    <tr
                      ...
                      className={clsx(
                        ...
                        isSelected && "!bg-[#FF00B6]/6 !border-l-[#FF00B6]/40",
                      )}
                    >
                      {/* Checkbox */}
                      <td
                      ...
                      <td className="px-4 py-3">
                        <span className="font-mono text-[11px] text-[#FF00B6]/80 tracking-wider group-hover:text-[#FF00B6] transition-colors">
                          #{order.order_number}
                        </span>
                      </td>
                      ...
                      <td className="px-4 py-3">
                        <div className="flex flex-col gap-0.5">
                          <span className="font-poppins text-[12px] text-white/65 font-medium leading-tight group-hover:text-white/85 transition-colors">
                            {order.full_name}
                          </span>
                          <span className="font-mono text-[9px] text-white/20 truncate max-w-[180px]">
                            {order.email}
                          </span>
                        </div>
                      </td>
                      ...
                      {/* Total */}
                      <td className="px-4 py-3 whitespace-nowrap">
                        <span className="font-mono text-[13px] text-white/75 font-bold tabular-nums group-hover:text-white/90 transition-colors">
                      ...
                      <td className="px-4 py-3">
                        <span className="font-mono text-[9px] text-white/12">—</span>
                      </td>
                      ...
                      >
                        <Link
                          ...
                          className="inline-flex items-center gap-1 font-mono text-[8px] uppercase tracking-widest text-white/20 hover:text-[#00F0FF] border border-white/8 hover:border-[#00F0FF]/30 hover:shadow-[0_0_8px_rgba(0,240,255,0.15)] px-2 py-1 transition-all duration-150"
                        >
                          <Eye size={10} />
                          Ver
                        </Link>
      ...
      {(result.totalPages > 1 || paymentFilter) && (
        <div className="shrink-0 border-t border-white/5 px-6 py-3 flex items-center justify-between gap-4 flex-wrap">
          <div className="flex items-center gap-3">
            <span className="font-mono text-[9px] uppercase tracking-widest text-white/20">
              Pág. {currentPage} / {result.totalPages}
              {" · "}
              {result.total.toLocaleString("pt-BR")} pedidos
            </span>
            {paymentFilter && (
              <span className="font-mono text-[8px] uppercase tracking-widest text-[#FFD700]/40">
                Filtro de pagamento ativo (client-side)
              </span>
            )}
          </div>
          ...
          <div className="flex items-center gap-1.5">
            ...
            <div className="flex items-center gap-1.5 mr-3">
              <span className="font-mono text-[8px] uppercase tracking-widest text-white/12">por pág.</span>
            ...
            {pageNumbers.map((p, idx) =>
              p === "…" ? (
                <span key={`ellipsis-${idx}`} className="font-mono text-[10px] text-white/15 px-1">
                  …
                </span>
              ) : (
                <button
                  ...
                  className={clsx(
                    "w-7 h-7 font-mono text-[10px] border transition-all duration-150",
                    p === currentPage
                      ? "border-[#FF00B6] text-[#FF00B6] bg-[#FF00B6]/8 shadow-[0_0_6px_rgba(255,0,182,0.2)]"
