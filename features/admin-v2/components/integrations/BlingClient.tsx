 ...
 *
 * Usado em: src/app/admin-v2/integrations/bling/page.tsx.
 */
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-2">
        ...
        <div className="flex items-center gap-3">
          <Zap size={18} className="text-[#FF00B6]" />
      ...
      {/* Status card */}
      <div className="border border-white/8 bg-[#141414] p-6">
        <div className="flex items-start justify-between gap-6 flex-wrap">
          <div className="flex items-start gap-4">
            <div className="w-14 h-14 border border-white/8 bg-[#0f0f0f] flex items-center justify-center shrink-0">
              <Zap size={22} className="text-white/15" />
            </div>
            <div>
              <p className="font-mono text-[14px] text-white font-bold mb-1">Bling ERP</p>
              <p className="font-mono text-[9px] text-white/30 leading-relaxed max-w-sm mb-3">
          ...
          <div className="flex flex-col gap-2 shrink-0">
            <button
              type="button"
              disabled
              className="flex items-center gap-2 px-5 py-2.5 font-mono text-[10px] uppercase tracking-widest border border-white/8 text-white/15 cursor-not-allowed"
            >
              <Lock size={11} />
              Conectar Bling
            </button>
            <a
              ...
              className="flex items-center justify-center gap-1.5 px-3 py-1.5 font-mono text-[8px] uppercase tracking-widest border border-white/5 text-white/20 hover:text-white/35 hover:border-white/10 transition-colors"
            >
              <ExternalLink size={9} />
              Documentação API
            </a>
      ...
      <div className="border border-[#FFD700]/20 bg-[#FFD700]/4 p-4">
        <p className="font-mono text-[10px] text-[#FFD700]/70 uppercase tracking-wider mb-3">
          Pré-requisitos para ativação
        </p>
        <div className="space-y-2">
          {PREREQS.map((item) => (
            <div key={item} className="flex items-start gap-2">
              <Circle size={9} className="text-[#FFD700]/30 shrink-0 mt-0.5" />
              <p className="font-mono text-[9px] text-white/30 leading-relaxed">{item}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="border border-white/5 bg-[#0a0a0a] p-5">
        <p className="font-mono text-[8px] uppercase tracking-[0.3em] text-white/20 mb-4">
          Fluxo de dados
        </p>
        <div className="space-y-2 font-mono text-[9px] text-white/30 leading-loose">
          <p>
            <span className="text-[#00F0FF]/50">Checkout pago</span>
            <span className="text-white/15 mx-2">──►</span>
            /api/webhooks/mercadopago
            <span className="text-white/15 mx-2">──►</span>
            <span className="text-[#FF00B6]/40">createOrderInBling()</span>
            <span className="text-white/15 mx-2">──►</span>
            <span className="text-[#FFD700]/40">Bling gera NF-e</span>
          </p>
          <p>
            <span className="text-[#00F0FF]/50">Bling altera estoque</span>
            <span className="text-white/15 mx-2">──►</span>
            /api/webhooks/bling
            <span className="text-white/15 mx-2">──►</span>
            <span className="text-[#FF00B6]/40">atualiza product_variants.stock</span>
          </p>
        </div>
      </div>
      ...
      <div>
        <p className="font-mono text-[9px] uppercase tracking-[0.3em] text-white/20 mb-4">
          Plano de implementação — 4 fases
        </p>
        <div className="space-y-3">
          {PHASES.map((phase) => (
            <div key={phase.num} className="border border-white/5 bg-[#141414]">
              <div className="flex items-center gap-3 px-4 py-3 border-b border-white/5">
                <span className="w-6 h-6 border border-white/8 flex items-center justify-center font-mono text-[9px] text-white/30 shrink-0">
                  {phase.num}
                </span>
                <ArrowRight size={10} className="text-white/10 shrink-0" />
                <span className="font-mono text-[11px] text-white/60 font-bold">
                  {phase.title}
                </span>
                <span className="ml-auto px-2 py-0.5 font-mono text-[7px] uppercase tracking-widest border border-white/5 text-white/15">
                  Pendente
                </span>
              </div>
              <div className="px-4 py-3 space-y-1.5">
                {phase.items.map((item) => (
                  <div key={item} className="flex items-start gap-2">
                    <Circle size={8} className="text-white/12 shrink-0 mt-0.5" />
      ...
      <div className="border border-white/5 p-4 opacity-30">
        <p className="font-mono text-[9px] uppercase tracking-[0.3em] text-white/25 mb-3 flex items-center gap-2">
        ...
        <div className="grid grid-cols-2 gap-2">
          {[
            ...
            <div
              key={item}
              className="border border-white/5 bg-[#0f0f0f] px-3 py-2 font-mono text-[8px] text-white/20"
            >
              {item}
            </div>
          ))}
