...
export function ShippingZonesClient() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* ME notice */}
      <div className="border border-brand-pink/15 bg-brand-pink/4 p-4 flex items-start gap-3">
        <Truck size={17} className="text-brand-pink/50 shrink-0 mt-0.5" />
        <div className="space-y-1">
          <p className="font-mono text-[12px] text-brand-pink/60 uppercase tracking-wider">
          ...
          <p className="font-mono text-[11px] text-white/30 leading-relaxed max-w-xl">
            ...
            <span className="text-brand-pink/50">shipping_zones</span>.
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        {[
          ...
          <div key={label} className="border border-brand-pink/30 bg-[#050505] px-5 py-4 shadow-[inset_0_0_15px_rgba(255,0,182,0.05)]">
          ...
          >
            <div className="flex items-start justify-between mb-3">
              <div className="flex items-center gap-2">
                <MapPin size={17} className="text-brand-pink/60 shrink-0" />
              ...
              <div className="flex items-center gap-2 shrink-0">
                {zone.status === "limited" && (
                  <span className="px-2 py-0.5 font-mono text-[9px] uppercase tracking-widest border border-[#FFD700]/30 text-[#FFD700]/60">
                    Limitado
                  </span>
                )}
                <span className="px-2 py-0.5 font-mono text-[10px] uppercase tracking-widest border border-white/8 text-white/25">
                  {zone.region}
                </span>
              </div>
            </div>
            ...
            <div className="flex flex-wrap gap-1 mb-3">
              {zone.states.map((s) => (
                <span
                  key={s}
                  className="px-2 py-0.5 font-mono text-[9px] uppercase tracking-wider border border-white/5 text-white/20 bg-white/1"
                >
                  {s}
                </span>
              ))}
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-2 pt-3 border-t border-white/5">
              <div>
                <p className="font-mono text-[9px] uppercase tracking-wider text-white/15 mb-0.5">
                  Prazo
                </p>
                <p className="font-mono text-[13px] text-white/40">{zone.avg_days} dias</p>
              </div>
              <div>
                <p className="font-mono text-[9px] uppercase tracking-wider text-white/15 mb-0.5">
                  Frete médio
                </p>
                <p className="font-mono text-[13px] text-white/40">{zone.avg_price}</p>
              </div>
              <div>
                <p className="font-mono text-[9px] uppercase tracking-wider text-white/15 mb-0.5">
                  % Pedidos
                ...
                <div className="flex items-center gap-2">
                  <p className="font-mono text-[13px] text-white/40">{zone.orders_pct}%</p>
                  <div className="flex-1 h-1 bg-white/5 overflow-hidden">
                    <div
                      className="h-full bg-brand-pink/30"
            ...
            <div className="mt-3 pt-3 border-t border-white/5">
              <p className="font-mono text-[9px] uppercase tracking-wider text-white/15 mb-1.5">
                Transportadoras
              </p>
              <div className="flex flex-wrap gap-1">
                {zone.carriers.map((c) => (
                  <span
                    key={c}
                    className="flex items-center gap-1 px-2 py-0.5 font-mono text-[9px] uppercase tracking-wider border border-brand-pink/10 text-brand-pink/30"
                  >
                    <Package size={11} />
                    {c}
                  </span>
            ...
            <button
              type="button"
              disabled
              className="mt-3 w-full flex items-center justify-center gap-1.5 py-1.5 font-mono text-[10px] uppercase tracking-widest border border-white/5 text-white/12 cursor-not-allowed"
            >
              <Lock size={12} />
              [ CONFIGURAR ZONA ]
            </button>
