...
export function WishlistListClient({ items }: Props) {
  ...
  function SortChevron({ k }: { k: SortKey }) {
    if (k !== sortKey) return <ArrowUpDown size={12} className="text-white/20" />;
    return sortDir === "desc" ? (
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-2">
        ...
        <p className="font-mono text-[11px] uppercase tracking-[0.3em] text-white/25">{"// "}{items.length} produtos favoritados — somente leitura
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        {[
          ...
          >
            ...
            <p className="font-mono text-base text-white/80 truncate">{kpi.value}</p>
          </div>
        ))}
      </div>
      ...
      <div className="relative max-w-sm">
        ...
        <input
          ...
          className="w-full pl-9 pr-4 py-2.5 bg-[#0a0a0a] border border-white/8 font-mono text-[13px] text-white/70 placeholder:text-white/20 focus:outline-none focus:border-brand-pink/40 transition-all"
        />
      </div>
              ...
              filtered.map((row, i) => (
                ...
                >
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-3">
                      <div className="w-9 h-9 shrink-0 border border-white/5 bg-[#050505] overflow-hidden">
                        {row.product_thumb ? (
                          ...
                          <div className="w-full h-full flex items-center justify-center">
                            <span className="font-mono text-[9px] text-white/15">IMG</span>
                          </div>
                        )}
                      </div>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      <Heart size={14} className="text-brand-pink/50 fill-[var(--brand-pink)]/30" />
                      <span className="font-mono text-[15px] text-brand-pink">
                        {row.total_favorites}
                      </span>
                    </div>
                  </td>
                  <td className="px-4 py-3 font-mono text-[12px] text-white/30">{"// "}{fmtDate(row.last_added)}
                  </td>
                </tr>
              ))
            )}
      ...
      <p className="font-mono text-[10px] text-white/12 text-center uppercase tracking-[0.3em]">{"// "}{filtered.length} / {items.length} produtos — somente leitura
      </p>
    </div>
  );
}
