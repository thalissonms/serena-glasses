...
export default function ProductPageContent({ product, videoSrc }: ProductPageContentProps) {
  const [selectedColor, setSelectedColor] = useState(0);

  return (
    <main
      className="w-full min-h-screen text-black dark:text-white pb-10 bg-[#FFF0FA] dark:bg-[#0a0a0a] transition-colors"
      style={{
    ...
    >
      <section className="max-w-[90vw] mx-auto px-20 py-10 grid grid-cols-1 md:grid-cols-[26.5rem_1fr] items-start">
        <ProductMediaViewer
          name={product.name}
          videoSrc={videoSrc}
          fallbackImageUrl={product.images.find((img) => img.isPrimary)?.url ?? product.images[0]?.url}
        />

        <div className="flex-1 flex flex-col gap-3 pl-20 py-4">
          <ProductInfo
            product={product}
            selectedColor={selectedColor}
            onColorChange={setSelectedColor}
          />
          <ProductActions product={product} selectedColorIndex={selectedColor} />
          <ProductDelivery />
        </div>
      </section>
